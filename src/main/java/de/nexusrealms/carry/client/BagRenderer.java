...
public class BagRenderer implements TrinketRenderer {
    ...
    private final Supplier<TexturedModelData> dyeLayer;
    ...
    public void render(ItemStack stack, SlotReference slotReference, EntityModel<? extends LivingEntityRenderState> contextModel, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light, LivingEntityRenderState state, float limbAngle, float limbDistance) {
        if(baseModel == null){
            baseModel = new SatchelModel(baseLayer.get().createModel());
        }
        if(dyeModel == null){
            dyeModel = new SatchelModel(dyeLayer.get().createModel());
        }
        if(strapModel == null){
        ...
        baseModel.render(matrices, vertexConsumers.getBuffer(RenderLayer.getEntityTranslucent(baseTexture)), light, OverlayTexture.DEFAULT_UV, 0xffffffff);
